...
# Configuración: Resonancia 0.3959 con Proyección Activa
...
# Motor con proyección habilitada
engine = CrecimientoFractal(harmonics=h_dna, dual_strand=True, target_dist=20.0)
sim_coords = engine.generar_espiral_aurea(num_nodos=num_nodos, aplicar_proyeccion=True)

# Validación
s_norm = normalizar_coords(sim_coords[:num_nodos])
error = np.sqrt(np.mean((s_norm - r_norm)**2))

print(f"--- ANÁLISIS DE PROYECCIÓN FÍSICA ---")
print(f"Estructura: 1BNA (ADN)")
print(f"RMSD Proyectado: {error:.4f}")
print(f"Estado del Sistema: {'Sincronizado' if error < 0.1 else 'Desfasado'}")